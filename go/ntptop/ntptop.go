...
func main() {
        ...
        // SYNC: Set the initial UI state to match the command-line flag
        displayMu.Lock()
        showDNS = *enableDNS
        displayMu.Unlock()
...
func (c *DNSCache) Lookup(ip string) (string, string, string) {
        ...
        // FIX: Als DNS nu aan staat, maar de cache bevat nog een kale IP (omdat DNS
        // uit stond tijdens de eerste lookup), dwingen we een hernieuwde lookup af.
        dnsNeedsRefresh := *enableDNS && (!nameExists || name == ip)

        if nameExists && asnExists && geoExists && !dnsNeedsRefresh {
                return name, asn, geo
        }

        // Als we moeten refreshen of data missen, sturen we hem (opnieuw) naar de queue
        select {
        case c.queue <- ip:
        default:
        }
...
func dnsWorker(ctx context.Context) {
        for {
                ...
                case ip := <-dnsCache.queue:
                        dnsCache.mu.RLock()
                        name, nameExists := dnsCache.cache[ip]
                        _, asnExists := dnsCache.asn[ip]
                        _, geoExists := dnsCache.geo[ip]
                        dnsCache.mu.RUnlock()

                        // FIX: Ook hier controleren we of we alsnog DNS moeten doen
                        dnsNeedsRefresh := *enableDNS && (!nameExists || name == ip)
                        ...
                        // 2. DNS lookup (if enabled and not yet successfully resolved)
                        if !nameExists || name == ip {
                                resolved := ip
                                if *enableDNS {
                                ...
                                // Sla het resultaat alleen op als we óf DNS succesvol hebben gedaan,
                                // óf als er nog helemaal niets in de cache stond (om een lege loop te voorkomen).
                                if *enableDNS || !nameExists {
                                        dnsCache.mu.Lock()
